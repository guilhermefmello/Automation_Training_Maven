...
public class Scenario_1_Step_Definition {
	...
	public void user_on_products_page(){
		...
		WebElement productsPage = driver.findElement(By.className("Products"));
		System.out.println("After login user arrives at page: " + productsPage);
		Assert.assertEquals("Products", productsPage);
		
	}
	
	
	// Verifying that the user is logged in Saucedemo.com Website
...
//			System.out.println("The following user is logged in: " + loggedIn);
		...
		boolean user_on_products_page = true;

		if (user_on_products_page) {

			WebElement loggedIn = driver.findElement(By.linkText("https://www.saucedemo.com/inventory.html"));
			System.out.println("The user is logged in: " + loggedIn);
